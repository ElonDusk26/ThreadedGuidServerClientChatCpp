...
char getch(bool block = false) {
        ...
        if (tcgetattr(0, &old) < 0)
                perror("tcsetattr()");
        old.c_lflag &= ~ICANON; //devilish bitwise operations: & bitwise AND (removes bits)
        old.c_lflag &= ~ECHO; // ~ bitwise NOT operator (inverts like ! but bitwise). translated to "keep all but ECHO flag"
        old.c_cc[VMIN] = 1;
        old.c_cc[VTIME] = 0;
        ...
        if (read(0, &buf, 1) < 0)
                //perror ("read()");
        old.c_lflag |= ICANON; //devilish bitwise operations: | bitwise OR (adds bits)
        old.c_lflag |= ECHO; //translated "add ECHO"
